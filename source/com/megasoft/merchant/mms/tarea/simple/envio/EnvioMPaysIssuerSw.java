...
/**
 * Clase encargada de extraer los MPaysIssuerSw de las bases de datos Merchant
 * Server
 * 
...
public class EnvioMPaysIssuerSw extends Envio {

	private Logger logger = Logger.getLogger(EnvioMPaysIssuerSw.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioMPaysIssuerSw");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioMPaysIssuerSw");
			}
		}
	}

	private boolean envioMPaysIssuerSw() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioMPaysIssuerSw");


		QueryConfigResult mPaysIssuerSw;

		try {
			mPaysIssuerSw = opGetBD.getMPaysIssuerSwMQ();
			if (mPaysIssuerSw == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getMPaysIssuerSwMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado MPaysIssuerSw con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("mPaysIssuerSw");
			...
			String nombreProcesamientoServerMMS = "MPaysIssuerSw";

			String nombreTabla = "MPaysIssuerSw";

			int cantidadDeRegistros = mPaysIssuerSw.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, mPaysIssuerSw.getQueryExecutionTime(), mPaysIssuerSw.getJsonExecutionTime(), 0, excludingColumns, mPaysIssuerSw.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de MPaysIssuerSw a enviar son: " + mPaysIssuerSw.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("mPaysIssuerSw", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los MPaysIssuerSw, cantidad: " + mPaysIssuerSw.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
