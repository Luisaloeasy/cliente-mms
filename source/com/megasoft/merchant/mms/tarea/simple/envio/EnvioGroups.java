...
/**
 * Clase encargada de extraer los Groups de las bases de datos Merchant Server
 * 
 * @author Luis Alejandro Aloisi Millan
 *
 */

public class EnvioGroups extends Envio {

	private Logger logger = Logger.getLogger(EnvioGroups.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioGroups");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioGroups");
			}
		}
	}

	private boolean envioGroups() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioGroups");


		QueryConfigResult groups;

		try {
			groups = opGetBD.getGroupsMQ();
			if (groups == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getGroupsMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado Groups con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("groups");
			...
			String nombreProcesamientoServerMMS = "Groups";

			String nombreTabla = "Groups";

			int cantidadDeRegistros = groups.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, groups.getQueryExecutionTime(), groups.getJsonExecutionTime(), 0, excludingColumns, groups.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de Groups a enviar son: " + groups.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("groups", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los Groups, cantidad: " + groups.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
