...
/**
 * Clase encargada de extraer los UniversalBins de las bases de datos Merchant
 * Server
 * 
...
public class EnvioUniversalBins extends Envio {

	private Logger logger = Logger.getLogger(EnvioUniversalBins.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioUniversalBins");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioUniversalBins");
			}
		}
	}

	private boolean envioUniversalBins() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioUniversalBins");

		QueryConfigResult universalBins;

		try {
			universalBins = opGetBD.getUniversalBinsMQ();
			if (universalBins == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getUniversalBinsMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado UniversalBins con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("universalBins");
			...
			String nombreProcesamientoServerMMS = "UniversalBins";

			String nombreTabla = "UniversalBins";

			int cantidadDeRegistros = universalBins.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, universalBins.getQueryExecutionTime(), universalBins.getJsonExecutionTime(), 0, excludingColumns, universalBins.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de UniversalBins a enviar son: " + universalBins.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("universalBins", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los UniversalBins, cantidad: " + universalBins.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
