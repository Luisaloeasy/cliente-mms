...
/**
 * Clase encargada de extraer los VTerminalTerminals de las bases de datos
 * Merchant Server
 * 
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioVTerminalTerminals extends Envio {

	private Logger logger = Logger.getLogger(EnvioVTerminalTerminals.class.getName());
	...
	public EnvioVTerminalTerminals() {
		super();
	}
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioVTerminalTerminals");
		}
		...
		try {
			...
			envioVTerminalTerminals();

		} catch (Exception e) {
			logger.error("MMS v2 -- Error no identificado '" + e.getMessage() + "'", e);
		} finally {
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioVTerminalTerminals");
			}
		}
	}

	private boolean envioVTerminalTerminals() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioVTerminalTerminals");

		QueryConfigResult vTerminalTerminals;

		try {
			vTerminalTerminals = opGetBD.getVTerminalTerminalsMQ();
			if (vTerminalTerminals == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getVTerminalTerminalsMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado VTerminalTerminals con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("vTerminalTerminals");
			...
			String nombreProcesamientoServerMMS = "VTerminalTerminals";

			String nombreTabla = "VTerminalTerminals";

			int cantidadDeRegistros = vTerminalTerminals.getSize();
			...
			if (dataEqualToPreviousExtraction) {
				...
				vTerminalTerminals.clear();
			}

			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, vTerminalTerminals.getQueryExecutionTime(), vTerminalTerminals.getJsonExecutionTime(), 0, excludingColumns, vTerminalTerminals.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de vTerminalTerminals a enviar son: " + vTerminalTerminals.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("vTerminalTerminals", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los vTerminalTerminals, cantidad: " + vTerminalTerminals.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
