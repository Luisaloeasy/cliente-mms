...
/**
 * Clase encargada de extraer los Cards de las bases de datos Merchant Server
 * 
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioCards extends Envio {

	private Logger logger = Logger.getLogger(EnvioCards.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioCards");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioCards");
			}
		}
	}

	private boolean envioCards() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioCards");

		QueryConfigResult cards;

		try {
			cards = opGetBD.getCardsMQ();
			if (cards == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getCardsMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado Cards con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("cards");
			...
			String nombreProcesamientoServerMMS = "Cards";

			String nombreTabla = "Cards";

			int cantidadDeRegistros = cards.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, cards.getQueryExecutionTime(), cards.getJsonExecutionTime(), 0, excludingColumns, cards.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de Cards a enviar son: " + cards.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("cards", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los Cards, cantidad: " + cards.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
