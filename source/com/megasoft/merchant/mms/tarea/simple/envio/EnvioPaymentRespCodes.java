...
/**
 * Clase encargada de extraer los PaymentRespCodes de las bases de datos
 * Merchant Server
 *
...
public class EnvioPaymentRespCodes extends Envio {

	private Logger logger = Logger.getLogger(EnvioPaymentRespCodes.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioPaymentRespCodes");
		}
		...
		try {
			...
			envioPaymentRespCodes();

		} catch (Exception e) {
			logger.error("MMS v2 -- Error no identificado '" + e.getMessage() + "'", e);
		} finally {
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioPaymentRespCodes");
			}
		}
	}

	private boolean envioPaymentRespCodes() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioPaymentRespCodes");

		QueryConfigResult paymentRespCodes;

		try {
			paymentRespCodes = opGetBD.getPaymentRespCodeMQ();
			if (paymentRespCodes == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getPaymentRespCodeMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado PaymentRespCodes con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("paymentRespCodes");
			...
			String nombreProcesamientoServerMMS = "paymentRespCodes";

			String nombreTabla = "paymentRespCodes";

			int cantidadDeRegistros = paymentRespCodes.getSize();
			...
			if (dataEqualToPreviousExtraction) {
				...
				paymentRespCodes.clear();
			}

			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, paymentRespCodes.getQueryExecutionTime(), paymentRespCodes.getJsonExecutionTime(), 0, excludingColumns, paymentRespCodes.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de paymentRespCodes a enviar son: " + paymentRespCodes.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("paymentRespCodes", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los paymentRespCodes, cantidad: " + paymentRespCodes.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
