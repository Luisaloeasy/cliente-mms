...
/**
 * Clase encargada de extraer los Banks de las bases de datos Merchant Server
 *
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioBanks extends Envio {
	
	private Logger logger = Logger.getLogger(EnvioBanks.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioBanks");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioBanks");
			}
		}
	}

	private boolean envioBanks() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioBanks");

		QueryConfigResult banks;

		try {
			banks = opGetBD.getBanksMQ();
			if (banks == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getBanksMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado Banks con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("banks");
			...
			String nombreProcesamientoServerMMS = "Banks";

			String nombreTabla = "Banks";

			int cantidadDeRegistros = banks.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, banks.getQueryExecutionTime(), banks.getJsonExecutionTime(), 0, excludingColumns, banks.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de Banks a enviar son: " + banks.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("banks", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los Banks, cantidad: " + banks.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
