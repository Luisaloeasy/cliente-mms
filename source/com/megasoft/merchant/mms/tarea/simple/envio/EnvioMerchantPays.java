...
/**
 * Clase encargada de extraer los MerchantPays de las bases de datos Merchant
 * Server
 * 
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioMerchantPays extends Envio {

	private Logger logger = Logger.getLogger(EnvioMerchantPays.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioMerchantPays");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioMerchantPays");
			}
		}
	}

	private boolean envioMerchantPays() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioMerchantPays");

		QueryConfigResult merchantPays;

		try {
			merchantPays = opGetBD.getMerchantPaysMQ();
			if (merchantPays == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getMerchantPaysMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado MerchantPays con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("merchantPays");
			...
			String nombreProcesamientoServerMMS = "MerchantPays";

			String nombreTabla = "MerchantPays";

			int cantidadDeRegistros = merchantPays.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, merchantPays.getQueryExecutionTime(), merchantPays.getJsonExecutionTime(), 0, excludingColumns, merchantPays.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de MerchantPays a enviar son: " + merchantPays.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("merchantPays", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los MerchantPays, cantidad: " + merchantPays.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
