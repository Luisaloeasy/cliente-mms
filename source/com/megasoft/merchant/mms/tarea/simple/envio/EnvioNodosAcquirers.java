...
/**
 * Clase encargada de extraer los NodosAcquirers de las bases de datos Merchant Server
 *
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioNodosAcquirers extends Envio {

    private Logger logger = Logger.getLogger(EnvioNodosAcquirers.class.getName());
    ...
    public void run(Map parameters) throws Exception {
        ...
        if (logger.isInfoEnabled()) {
            ...
            logger.info("MMS v2 -- Inicio Tarea EnvioNodosAcquirers");
        }
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("MMS v2 -- Termino Tarea EnvioNodosAcquirers");
            }
        }
    }

    private boolean envioNodosAcquirers() throws Exception {

        logger.info("MMS v2 -- Inicio Tarea EnvioNodosAcquirers");

        QueryConfigResult nodosAcquirers;

        try {
            nodosAcquirers = opGetBD.getNodosAcquirersMQ();
            if (nodosAcquirers == null) {
                logger.error("MMS v2 -- No hay datos disponibles en getNodosAcquirersMQ()");
                return false;
            }
            ...
            logger.error("MMS v2 -- Error inesperado NodosAcquirers con la consulta a la BD", e1);
            return false;
        }
        ...
        try {
            ...
            boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("nodosAcquirers");
            ...
            String nombreProcesamientoServerMMS = "NodosAcquirers";

            String nombreTabla = "NodosAcquirers";

            int cantidadDeRegistros = nodosAcquirers.getSize();
            ...
            if (dataEqualToPreviousExtraction) {
                ...
                nodosAcquirers.clear();
            }

            TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, nodosAcquirers.getQueryExecutionTime(), nodosAcquirers.getJsonExecutionTime(), 0, excludingColumns, nodosAcquirers.getResultadoFinal());

            if (logger.isDebugEnabled()) {
                logger.debug("MMS v2 -- La cantidad de NodosAcquirers a enviar son: " + nodosAcquirers.getSize());
            }

            ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

            tareaUtil.renameJsons("nodosAcquirers", dataEqualToPreviousExtraction);

            if (logger.isInfoEnabled()) {
                logger.info("MMS v2 -- Se enviaron correctamente los NodosAcquirers, cantidad: " + nodosAcquirers.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
            }
