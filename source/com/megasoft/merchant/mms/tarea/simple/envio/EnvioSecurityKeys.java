...
/**
 * Clase encargada de extraer los SecurityKeys de las bases de datos Merchant
 * Server
 * 
...
public class EnvioSecurityKeys extends Envio {

	private Logger logger = Logger.getLogger(EnvioSecurityKeys.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioSecurityKeys");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioSecurityKeys");
			}
		}
	}

	private boolean envioSecurityKeys() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioSecurityKeys");

		QueryConfigResult securityKeys;

		try {
			securityKeys = opGetBD.getSecurityKeysMQ();
			if (securityKeys == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getSecurityKeysMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado SecurityKeys con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("securityKeys");
			...
			String nombreProcesamientoServerMMS = "SecurityKeys";

			String nombreTabla = "SecurityKeys";

			int cantidadDeRegistros = securityKeys.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, securityKeys.getQueryExecutionTime(), securityKeys.getJsonExecutionTime(), 0, excludingColumns, securityKeys.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de SecurityKeys a enviar son: " + securityKeys.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("securityKeys", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los SecurityKeys, cantidad: " + securityKeys.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
