...
/**
 * Clase encargada de extraer los Medios de Pago de las bases de datos Merchant Server
 *
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioMedioPago extends Envio{

    private Logger logger = Logger.getLogger(EnvioMedioPago.class.getName());
    ...
    public void run(Map parameters) throws Exception {
        ...
        if (logger.isInfoEnabled()) {
            ...
            logger.info("MMS v2 -- Inicio Tarea EnvioMedioPago");
        }
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("MMS v2 -- Termino Tarea EnvioMedioPago");
            }
        }
    }

    private boolean envioMedioPago() throws Exception {

        logger.info("MMS v2 -- Inicio Tarea EnvioMedioPago");

        QueryConfigResult medioPagos;

        try {
            medioPagos = opGetBD.getMedioPagoMQ();
            if (medioPagos == null) {
                logger.error("MMS v2 -- No hay datos disponibles en getMedioPagoMQ()");
                return false;
            }
            ...
            logger.error("MMS v2 -- Error inesperado MedioPagos con la consulta a la BD", e1);
            return false;
        }
        ...
        try {
            ...
            boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("medioPagos");
            ...
            String nombreProcesamientoServerMMS = "MedioPago";

            String nombreTabla = "MedioPago";

            int cantidadDeRegistros = medioPagos.getSize();
            ...
            TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, medioPagos.getQueryExecutionTime(), medioPagos.getJsonExecutionTime(), 0, excludingColumns, medioPagos.getResultadoFinal());

            if (logger.isDebugEnabled()) {
                logger.debug("MMS v2 -- La cantidad de MedioPago a enviar son: " + medioPagos.getSize());
            }

            ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

            tareaUtil.renameJsons("medioPagos", dataEqualToPreviousExtraction);

            if (logger.isInfoEnabled()) {
                logger.info("MMS v2 -- Se enviaron correctamente los MedioPago, cantidad: " + medioPagos.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
            }
