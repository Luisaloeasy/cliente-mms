...
/**
 * Clase encargada de extraer transacciones de Batch de las bases de datos
 * Merchant Server
 * 
...
public class EnvioBatchMMS extends Envio {

	private Logger logger = Logger.getLogger(EnvioBatchMMS.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioBatchMMS");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioBatchMMS");
			}
		}
	}
	...
	private void intervalodetiempo() throws Exception {
		try {
			...
			Timestamp fechaInicio = tareaUtil.obtenerUltimaVezRealizada(formato, "MMS_v2_LastExecutionDateTimeBatch.xml");
			...
			do {
				...
				if (fechaFin.before(fechaActual)) {
					logger.info("MMS v2 -- La fechaFin esta antes de la fecha actual, se procede a ejecutar la tarea envioBatch");
					exito = envioBatch(fechaInicio, fechaFin);
					if (exito) {
						contadorEventos++;
					} else {
						logger.info("MMS v2 -- Ocurrio un error en la clase EnvioBatch, y no se actualiza el contadorEventos");
					}
					...
					if (contadorEventos == cantidadArchivosPorLote) {
						try {
							...
							logger.error("MMS v2 -- Ocurrio un error durmiendo el hilo BatchMMS", e);
						}
						contadorEventos = 0;
					...
					if (recuperarPorPeriodo && exito) {
						fechaInicio = tareaUtil.obtenerUltimaVezRealizada(formato, "MMS_v2_LastExecutionDateTimeBatch.xml");
					...
					debeContinuar = false;
					logger.info("MMS v2 -- La fechaFin esta despues de la fechaActual. Se pausa la tarea BatchMMS.");
				}
				endCalculateTimeStampExecutionTime = System.nanoTime();
				timestampRuntime = ((endCalculateTimeStampExecutionTime - startCalculateTimeStampExecutionTime) / 1e6);
			...
			logger.error("MMS v2 -- No se pudo cargar la informacion de Control de la Tarea (ConfigFiles/MMS_v2_LastExecutionDateTimeBatch.xml", e);
		}
		logger.info("MMS v2 -- Saliendo del ciclo. Se pausa el calculo de los intervalos.");
	}

	private boolean envioBatch(Timestamp fechaInicio, Timestamp fechaFin) throws Exception {

		startExecutionTime = System.nanoTime();

		if (logger.isDebugEnabled()) {
			logger.debug("MMS v2 -- Extraccion de Batch desde" + fechaInicio + " hasta " + fechaFin);
		}

		logger.info("MMS v2 -- Inicia proceso de extraccion BatchMMS");

		QueryMMSResult transactionsPending;

		try {
			transactionsPending = opGetBD.getBatch(fechaInicio, fechaFin);
			if (transactionsPending == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getBatch()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado Batch con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			String nombreTuplaProceso = "Batch";

			String nombreTabla = "Batch";

			int cantidadDeRegistros = transactionsPending.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date(fechaInicio.getTime())), formatoParaArchivo.format(new Date(fechaFin.getTime())), nombreTabla, nombreTuplaProceso, cantidadMensajesEnviar, runtime, transactionsPending.getQueryExecutionTime(), transactionsPending.getJsonExecutionTime(), timestampRuntime, excludingColumns, transactionsPending.getResultadoFinal());


			logger.info("MMS v2 -- La cantidad de transacciones pendiente de cierre son: " + transactionsPending.getSize() + ". Inicio del envio en la fecha: " + new Date());


			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);


			logger.info("MMS v2 -- Se enviaron correctamente las transacciones de Batch, cantidad: " + transactionsPending.getSize() + " desde " + fechaInicio + " hasta " + fechaFin + ". Runtime: " + runtime + " ms. Fin del envio de evento en la fecha: " + new Date());


			return tareaUtil.guardarUltimaVezRealizada(formato, "MMS_v2_LastExecutionDateTimeBatch.xml", "MMS_v2_LastExecutionDateTimeBatch", fechaFin);
