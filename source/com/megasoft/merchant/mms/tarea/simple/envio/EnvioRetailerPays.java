...
/**
 * Clase encargada de extraer los RetailerPays de las bases de datos Merchant
 * Server
 * 
...
public class EnvioRetailerPays extends Envio {

	private Logger logger = Logger.getLogger(EnvioRetailerPays.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioRetailerPays");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioRetailerPays");
			}
		}
	}

	private boolean envioRetailerPays() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioRetailerPays");

		QueryConfigResult retailerPays;

		try {
			retailerPays = opGetBD.getRetailerPaysMQ();
			if (retailerPays == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getRetailerPaysMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado RetailerPays con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("retailerPays");
			...
			String nombreProcesamientoServerMMS = "RetailerPays";

			String nombreTabla = "RetailerPays";

			int cantidadDeRegistros = retailerPays.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, retailerPays.getQueryExecutionTime(), retailerPays.getJsonExecutionTime(), 0, excludingColumns, retailerPays.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de RetailerPays a enviar son: " + retailerPays.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("retailerPays", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los RetailerPays, cantidad: " + retailerPays.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
