...
/**
 * Clase encargada de extraer History de las bases de datos Merchant Server
 * 
 * @author Luis Alejandro Aloisi Millan
 *
 */

public class EnvioHistoryMMS extends Envio {

	private Logger logger = Logger.getLogger(EnvioHistoryMMS.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioHistoryMMSOld");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioHistoryMMSOld");
			}
		}
	}
	...
	private void intervalodetiempo() throws Exception {
		try {
			...
			Timestamp fechaInicio = tareaUtil.obtenerUltimaVezRealizada(formato, "MMS_v2_LastExecutionDateTimeHistory.xml");
			...
			do {
				...
				if (fechaFin.before(fechaActual)) {
					logger.info("MMS v2 -- La fechaFin esta antes de la fecha actual, se procede a ejecutar la tarea envioHistory");
					exito = envioHistory(fechaInicio, fechaFin);
					if (exito) {
						contadorEventos++;
					} else {
						logger.info("MMS v2 -- Ocurrio un error en la clase EnvioHistory, y no se actualiza el contador de eventos");
					}
					...
					if (contadorEventos == cantidadArchivosPorLote) {
						try {
							...
							logger.error("MMS v2 -- Ocurrio un error durmiendo el hilo HistoryMMS", e);
						}
						contadorEventos = 0;
					...
					if (recuperarPorPeriodo && exito) {
						fechaInicio = tareaUtil.obtenerUltimaVezRealizada(formato, "MMS_v2_LastExecutionDateTimeHistory.xml");
					...
					debeContinuar = false;
					logger.info("MMS v2 -- La fechaFin esta despues de la fechaActual. Se pausa la tarea HistoryMMS.");
				}
				endCalculateTimeStampExecutionTime = System.nanoTime();
				timestampRuntime = ((endCalculateTimeStampExecutionTime - startCalculateTimeStampExecutionTime) / 1e6);
			...
			logger.error("MMS v2 -- No se pudo cargar la informacion de Control de la Tarea (ConfigFiles/MMS_v2_LastExecutionDateTimeHistory.xml", e);
		}
		logger.info("MMS v2 -- Saliendo del ciclo. Se pausa el calculo de los intervalos.");
	}

	private boolean envioHistory(Timestamp fechaInicio, Timestamp fechaFin) throws Exception {

		startExecutionTime = System.nanoTime();

		logger.info("MMS v2 -- Inicia proceso de extraccion HistoryMMS");

		QueryMMSResult transactionsClosed;

		try {
			transactionsClosed = opGetBD.getHistory(fechaInicio, fechaFin);
			if (transactionsClosed == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getHistory(), ocurrio un problema null");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado History con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			String nombreTuplaProceso = "History";

			String nombreTabla = "History";

			int cantidadDeRegistros = transactionsClosed.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date(fechaInicio.getTime())), formatoParaArchivo.format(new Date(fechaFin.getTime())), nombreTabla, nombreTuplaProceso, cantidadMensajesEnviar, runtime, transactionsClosed.getQueryExecutionTime(), transactionsClosed.getJsonExecutionTime(), timestampRuntime, excludingColumns, transactionsClosed.getResultadoFinal());

			logger.info("MMS v2 -- La cantidad de transacciones cerradas a enviar son: " + transactionsClosed.getSize() + ". Inicio del envio en la fecha: " + new Date());

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			logger.info("MMS v2 -- Se enviaron correctamente las transacciones de History, cantidad: " + transactionsClosed.getSize() + " desde " + fechaInicio + " hasta " + fechaFin + ". Runtime: " + runtime + " ms. Fin del envio de evento en la fecha: " + new Date());

			return tareaUtil.guardarUltimaVezRealizada(formato, "MMS_v2_LastExecutionDateTimeHistory.xml", "MMS_v2_LastExecutionDateTimeHistory", fechaFin);
