...
/**
 * Clase encargada de extraer los Retailers de las bases de datos Merchant
 * Server
 *
 * @author Luis Alejandro Aloisi Millán
 *
 */

public class EnvioRetailers extends Envio {

	private final Logger logger = Logger.getLogger(EnvioRetailers.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioRetailersold");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioRetailersold");
			}
		}
	}

	private boolean envioRetailers() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioRetailersold ");
		QueryConfigResult retailers;

		try {
			retailers = opGetBD.getRetailerMQ();
			if (retailers == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getRetailerMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado Retailers con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("retailers");
			...
			String nombreProcesamientoServerMMS = "Retailers";

			String nombreTabla = "Retailers";

			int cantidadDeRegistros = retailers.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, retailers.getQueryExecutionTime(), retailers.getJsonExecutionTime(), 0, excludingColumns, retailers.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de Retailers a enviar son: " + retailers.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("retailers", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los Retailers, cantidad: " + retailers.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
