...
/**
 * Clase encargada de extraer los RespCodes de las bases de datos Merchant
 * Server
 *
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioRespCode extends Envio {

	private Logger logger = Logger.getLogger(EnvioRespCode.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioRespCode");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea RespCode");
			}
		}
	}

	private boolean envioEnvioRespCodes() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioRespCodes");

		QueryConfigResult respCodes;

		try {
			respCodes = opGetBD.getRespCodeMQ();
			if (respCodes == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getRespCodeMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado RespCodes con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("respCodes");
			...
			String nombreProcesamientoServerMMS = "respCodes";

			String nombreTabla = "respCodes";

			int cantidadDeRegistros = respCodes.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, respCodes.getQueryExecutionTime(), respCodes.getJsonExecutionTime(), 0, excludingColumns, respCodes.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de respCodes a enviar son: " + respCodes.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("respCodes", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los respCodes, cantidad: " + respCodes.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
