...
/**
 * Clase encargada de extraer los BinTables de las bases de datos Merchant
 * Server
 * 
...
public class EnvioBinTables extends Envio {
	
	private Logger logger = Logger.getLogger(EnvioBinTables.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioBinTables");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioBinTables");
			}
		}
	}

	private boolean envioBinTables() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioBinTables");

		QueryConfigResult binTables;

		try {
			binTables = opGetBD.getBinTablesMQ();
			if (binTables == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getBinTablesMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado BinTables con la consulta a la BD", e1);
			return false;
		}
		try {
			LinkedHashMap confCliente;

			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("binTables");
			...
			String nombreProcesamientoServerMMS = "BinTables";

			String nombreTabla = "BinTables";

			int cantidadDeRegistros = binTables.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, binTables.getQueryExecutionTime(), binTables.getJsonExecutionTime(), 0, excludingColumns, binTables.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de BinTables a enviar son: " + binTables.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("binTables", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los BinTables, cantidad: " + binTables.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
