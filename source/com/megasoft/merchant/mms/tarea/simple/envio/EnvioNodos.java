...
/**
 * Clase encargada de extraer los Nodos de las bases de datos Merchant Server
 *
 * @author Luis Alejandro Aloisi Millan
 *
 */
public class EnvioNodos extends Envio {

    private Logger logger = Logger.getLogger(EnvioNodos.class.getName());
    ...
    public void run(Map parameters) throws Exception {
        ...
        if (logger.isInfoEnabled()) {
            ...
            logger.info("MMS v2 -- Inicio Tarea EnvioNodos");
        }
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("MMS v2 -- Termino Tarea EnvioNodos");
            }
        }
    }

    private boolean envioNodos() throws Exception {

        logger.info("MMS v2 -- Inicio Tarea EnvioNodos");

        QueryConfigResult nodos;

        try {
            nodos = opGetBD.getNodosMQ();
            if (nodos == null) {
                logger.error("MMS v2 -- No hay datos disponibles en getNodosMQ()");
                return false;
            }
            ...
            logger.error("MMS v2 -- Error inesperado Nodos con la consulta a la BD", e1);
            return false;
        }
        ...
        try {
            ...
            boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("nodos");
            ...
            String nombreProcesamientoServerMMS = "Nodos";

            String nombreTabla = "Nodos";

            int cantidadDeRegistros = nodos.getSize();
            ...
            TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, nodos.getQueryExecutionTime(), nodos.getJsonExecutionTime(), 0, excludingColumns, nodos.getResultadoFinal());

            if (logger.isDebugEnabled()) {
                logger.debug("MMS v2 -- La cantidad de Nodos a enviar son: " + nodos.getSize());
            }

            ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

            tareaUtil.renameJsons("nodos", dataEqualToPreviousExtraction);

            if (logger.isInfoEnabled()) {
                logger.info("MMS v2 -- Se enviaron correctamente los Nodos, cantidad: " + nodos.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
            }
