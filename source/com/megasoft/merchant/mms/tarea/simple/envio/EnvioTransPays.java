...
/**
 * Clase encargada de extraer los TransPays de las bases de datos Merchant Server
 * 
 * @author Luis Alejandro Aloisi Millan
 *
 */

public class EnvioTransPays extends Envio {

	private Logger logger = Logger.getLogger(EnvioTransPays.class.getName());
	...
	public void run(Map parameters) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			...
			logger.info("MMS v2 -- Inicio Tarea EnvioTransPays");
		}
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Termino Tarea EnvioTransPays");
			}
		}
	}

	private boolean envioTransPays() throws Exception {

		logger.info("MMS v2 -- Inicio Tarea EnvioTransPays");

		QueryConfigResult transPays;

		try {
			transPays = opGetBD.getTransPaysMQ();
			if (transPays == null) {
				logger.error("MMS v2 -- No hay datos disponibles en getTransPaysMQ()");
				return false;
			}
			...
			logger.error("MMS v2 -- Error inesperado TransPays con la consulta a la BD", e1);
			return false;
		}
		...
		try {
			...
			boolean dataEqualToPreviousExtraction = tareaUtil.compareJsonToResulSet("transPays");
			...
			String nombreProcesamientoServerMMS = "TransPays";

			String nombreTabla = "TransPays";

			int cantidadDeRegistros = transPays.getSize();
			...
			TokenDatosMQ datosAEnviar = new TokenDatosMQ(nombreCliente, idCliente, puerto, nombreProcesamientoServerMMS, cantidadDeRegistros, checkSum, formatoParaArchivo.format(new Date()), nombreTabla, nombreProcesamientoServerMMS, cantidadMensajesEnviar, runtime, transPays.getQueryExecutionTime(), transPays.getJsonExecutionTime(), 0, excludingColumns, transPays.getResultadoFinal());

			if (logger.isDebugEnabled()) {
				logger.debug("MMS v2 -- La cantidad de TransPays a enviar son: " + transPays.getSize());
			}

			ComunicacionRabbitMQ.enviar(metodoEnvio, datosAEnviar);

			tareaUtil.renameJsons("transPays", dataEqualToPreviousExtraction);

			if (logger.isInfoEnabled()) {
				logger.info("MMS v2 -- Se enviaron correctamente los TransPays, cantidad: " + transPays.getSize() + " a las " + formatoParaArchivo.format(new Date()) + ". Runtime: " + runtime + " ms");
			}
