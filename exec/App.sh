...
#
# Copyright (c) 1999, 2024 Tanuki Software, Ltd.
...
# Optimized for use with version 3.5.60 of the Wrapper.
#
...
systemdDetection() {
    result=`systemctl status 2>/dev/null`
    if [ $? -ne 0 ] ; then
        # Not present
        return 1
    fi
...
# Default location for the systemd service file
...
WRAPPER_SCRIPT_NAME=$APP_NAME; export WRAPPER_SCRIPT_NAME
WRAPPER_SCRIPT_VERSION="3.5.60"; export WRAPPER_SCRIPT_VERSION
...
launchinternal() {
    ...
    then 
        ...
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.daemonize=TRUE $APPNAMEPROP $ANCHORPROP $IGNOREPROP $STATUSPROP $COMMANDPROP $LOCKPROP $ADDITIONAL_PARA"
        eval "$COMMAND_LINE"
    else
...
console() {
    ...
    then
        ...
        # The string passed to eval must handles spaces in paths correctly.
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" $APPNAMEPROP $ANCHORPROP $STATUSPROP $COMMANDPROP $LOCKPROP $ADDITIONAL_PARA"
        eval "$COMMAND_LINE"
        COMMAND_EXIT_CODE=$?
...
sourceFiles() {
    if [ -n "$FILES_TO_SOURCE" ] ; then
        ...
        for file in $files
        do
            FIRST_CHAR=`echo $file | cut -c1,1`
            if [ "$FIRST_CHAR" != "/" -a "$FIRST_CHAR" != "~" ] ; then
                normalizeFilePath file $REALDIR/$file
            else
